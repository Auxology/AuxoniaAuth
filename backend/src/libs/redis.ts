    ...
    export const lockSendingCodeForRecovery = async(userId: string):Promise<void> => {
        try {
            await redis.setEx(`locked_sending_code_for_recovery:${userId}`, 60, 'locked');
        }
        catch(err){
            console.error('Failed to lock sending code for recovery', err);
        }
    }

    export const checkIfSendingCodeForRecoveryIsLocked = async(userId: string):Promise<boolean> => {
        try {
            const isLocked = await redis.exists(`locked_sending_code_for_recovery:${userId}`);
            ...
            console.error('Failed to check if sending code for recovery is locked', err);
            return true;
        }
    }
